...
if ( !function_exists('gist_default_theme_options') ) :
	function gist_default_theme_options() {
		...
		$default_theme_options = array(
			...
			'gist-footer-copyright'=> esc_html__('All Rights Reserved 2020','gist'),
...
function gist_customize_register( $wp_customize ) {
	...
	if ( !function_exists('gist_breadcrumb_selection_enable') ) :
		function gist_breadcrumb_selection_enable(){
			global $gist_theme_options;
			$enable_bc = absint($gist_theme_options['gist-extra-breadcrumb']);
			if( $enable_bc == true){
				return true;
			}
			else{
	...
	$wp_customize->add_control( 'gist_theme_options[gist-breadcrumb-text]', array(
		...
		'type'      => 'text',
		'active_callback'=> 'gist_breadcrumb_selection_enable',
		'priority'  => 25
	) );

	/*Show Breadcrumb Types Selection*/
	$wp_customize->add_setting( 'gist_theme_options[gist-breadcrumb-display-from-option]', array(
		'capability'        => 'edit_theme_options',
		'transport' => 'refresh',
		'default'           => $default['gist-breadcrumb-display-from-option'],
		'sanitize_callback' => 'gist_sanitize_select'
	) );
	$wp_customize->add_control( 'gist_theme_options[gist-breadcrumb-display-from-option]', array(
		'choices' => array(
			'theme-default'    => __('Theme Default Breadcrumb','gist'),
			'plugin-breadcrumb'    => __('Plugins Breadcrumb','gist')
		),
		'label'     => __( 'Select the Breadcrumb Show Option', 'gist' ),
		'description' => __('Theme has its own breadcrumb. If you want to use the plugin breadcrumb, you need to enable the breadcrumb on the respected plugins first. Check plugin settings and enable it.', 'gist'),
		'section'   => 'gist_extra_section',
		'settings'  => 'gist_theme_options[gist-breadcrumb-display-from-option]',
		'type'      => 'select',
		'priority'  => 25,
		'active_callback'=> 'gist_breadcrumb_selection_enable',
	) );
	...
	if ( !function_exists('gist_breadcrumb_selection_option') ) :
		function gist_breadcrumb_selection_option(){
			global $gist_theme_options;
			$enable_breadcrumb = absint($gist_theme_options['gist-extra-breadcrumb']);
			$breadcrumb_selection = esc_attr($gist_theme_options['gist-breadcrumb-display-from-option']);
	...
	/*callback functions breadcrumb*/
	if ( !function_exists('gist_breadcrumb_selection_plugin') ) :
		function gist_breadcrumb_selection_plugin(){
			global $gist_theme_options;
			$enable_breadcrumb_plugin = absint($gist_theme_options['gist-extra-breadcrumb']);
			$breadcrumb_selection_plugin = esc_attr($gist_theme_options['gist-breadcrumb-display-from-option']);
	...
	$wp_customize->add_control( 'gist_theme_options[gist-breadcrumb-text]', array(
		'label'     => __( 'Breadcrumb Text', 'gist' ),
		'description' => __( 'Write your own text in place of You are Here', 'gist' ),
		'section'   => 'gist_extra_section',
		'settings'  => 'gist_theme_options[gist-breadcrumb-text]',
		'type'      => 'text',
		'priority'  => 25,
		'active_callback' => 'gist_breadcrumb_selection_option',
	) );


	/*Show Breadcrumb From Plugins*/
	$wp_customize->add_setting( 'gist_theme_options[gist-breadcrumb-display-from-plugins]', array(
		'capability'        => 'edit_theme_options',
		'transport' => 'refresh',
		'default'           => $default['gist-breadcrumb-display-from-plugins'],
		'sanitize_callback' => 'gist_sanitize_select'
	) );
	$wp_customize->add_control( 'gist_theme_options[gist-breadcrumb-display-from-plugins]', array(
		'choices' => array(
			'yoast'    => __('Yoast SEO Breadcrumb','gist'),
			'rank-math'    => __('Rank Math Breadcrumb','gist'),
			'navxt'    => __('NavXT Breadcrumb','gist')
		),
		'label'     => __( 'Select the Breadcrumb From Plugins', 'gist' ),
		'description' => __('Theme has its own breadcrumb. If you want to use the plugin breadcrumb, you need to enable the breadcrumb on the respected plugins first. Check plugin settings and enable it.', 'gist'),
		'section'   => 'gist_extra_section',
		'settings'  => 'gist_theme_options[gist-breadcrumb-display-from-plugins]',
		'type'      => 'select',
		'priority'  => 25,
		'active_callback'=> 'gist_breadcrumb_selection_plugin',
	) );
...
* Customizer Styles
*/
function gist_customizer_css() {
    wp_enqueue_style('gist-customizer-css', get_template_directory_uri() . '/candidthemes/assets/css/customizer-style.css', array(), '1.0.0');
}
add_action( 'customize_controls_enqueue_scripts', 'gist_customizer_css' );
